...
//     try {
//       await loginRequest(email, password, rememberMe);   // ← uncomment + pass rememberMe
//       if (!rememberMe) {
//         moveTokensToSession();
//       }
//       toast.success("Signed in successfully!");
//       setAuthModalView(null);
//       await loadSession();   // this fetches the real user from /api/auth/me using the stored token
//       router.push("/");
//     } catch (error) {
...
//   try {
//     await registerRequest(name, email, password);   // ← uncomment
...
import { moveTokensToSession, getRefreshTokenValue, storeTokens } from '@/lib/api';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadSession = useCallback(async () => {
    // Check if any token exists before hitting the network
    if (typeof window !== 'undefined') {
      const accessToken =
  ...
  const login = async (email: string, password: string, rememberMe: boolean) => {
    try {
      // loginRequest already stores tokens internally via lib/auth.ts
  ...
  const logout = async () => {
    try {
      await logoutRequest(); // calls backend + clears tokens
    } catch (error) {
      console.error('Logout error', error);
    } finally {
      setUser(null);
  ...
  const hideAuthModal = () => setAuthModalView(null);
