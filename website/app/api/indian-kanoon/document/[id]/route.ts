...
export async function GET(
  ...
  try {
    ...
    // Fetch document from Indian Kanoon API
    const indianKanoonUrl = `https://api.indiankanoon.org/doc/${docId}/`;
    ...
    console.error('Indian Kanoon document API error:', error);
    return NextResponse.json(
      {
        error: 'Failed to fetch document from Indian Kanoon',
