...
class TestExternalInferenceAgent:
    ...
    def test_infer_success(self):
        ...
        result = self.agent.infer("fundamental rights", top_k=2)
        retrieved = result['retrieved_cases']
        
        assert len(retrieved) == 2
        assert all("similarity_score" in r for r in retrieved)
        assert all("title" in r for r in retrieved)          # ← changed
        assert all("case_id" in r for r in retrieved)        # ← changed
        
        # Scores should be in descending TRS order (new sorting key)
        assert retrieved[0]["trs"] >= retrieved[1]["trs"]
    ...
    def test_infer_top_k_larger_than_candidates(self):
        ...
        result = self.agent.infer("test query", top_k=10)
        retrieved = result['retrieved_cases']          # ← NEW
    ...
    def test_similarity_scores_in_valid_range(self):
        ...
        result = self.agent.infer("constitutional rights", top_k=3)
        retrieved = result['retrieved_cases']          # ← NEW
        
        for case in retrieved:
            score = case["similarity_score"]
            assert 0.0 <= score <= 1.0                    # now always [0,1] (cosine on normalized vectors)
    ...
    def test_metadata_preservation(self):
        """Test that candidate 'id' is correctly mapped to 'case_id' in the output schema. Note: The agent deliberately returns a clean, standardized schema. Extra fields like 'category', 'text', 'content' etc. are NOT preserved."""
        self.agent.build_index(self.sample_candidates, text_field="text")
        
        result = self.agent.infer("privacy rights", top_k=1)
        retrieved = result['retrieved_cases']
        
        # Metadata should be preserved (id → case_id)
        assert len(retrieved) == 1
        assert "case_id" in retrieved[0]
        assert retrieved[0]["case_id"] in [1, 2, 3, 4]   # original ids from sample data
    ...
    def test_different_text_field(self):
        ...
        self.agent.build_index(candidates, text_field="content")
        result = self.agent.infer("law", top_k=1)
        retrieved = result['retrieved_cases']
        
        assert len(retrieved) == 1
        assert "case_id" in retrieved[0]                     # ← changed
        assert retrieved[0]["case_id"] == 1                  # original id is mapped to case_id
        # "content" key is NOT preserved (by design in current agent)
    ...
    # Extra test addded later
    def test_infer_does_not_preserve_extra_fields(self):
        """Document that the agent returns a clean schema and drops extra candidate fields."""
        self.agent.build_index(self.sample_candidates, text_field="text")
        
        result = self.agent.infer("privacy rights", top_k=1)
        case = result['retrieved_cases'][0]
        
        # These keys should exist (standard schema)
        assert "case_id" in case
        assert "title" in case
        assert "trs" in case
        
        # These extra keys from sample_candidates should NOT exist anymore
