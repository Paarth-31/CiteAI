...
class ExternalInferenceAgent:
    ...
    def build_index(
        ...
        self.text_field = text_field  # Add this line so the agent remembers the field name

        if not candidates:
            raise ValueError("Candidates list cannot be empty")

        # Validate text field presence and type
    ...
    def infer(
        ...
        # Accept plain string or dict target
        if isinstance(target, str):
            target_text = target
            # Overwrite the target variable with a dictionary structure
            target = {
                'text': target_text,
                'title': 'Query: ' + target_text[:30],  # Give it a fallback title for the summary
                'year': None,
        ...
        for candidate_idx, similarity_score in retrieved_candidates:
            ...
            # Use the dynamically saved text_field, falling back to 'text' if not found
            field_to_use = getattr(self, 'text_field', 'text')
            candidate_text = candidate[field_to_use]
            ...
            # Build result dict
            result_case = {
                "case_id": candidate.get('case_id', candidate.get('id', f"candidate_{candidate_idx}")),
                "title": candidate.get('title', candidate.get('text', '')[:60]),
        ...
        result = {
            "target": {
                # Safely get case_id and title with fallbacks
                "case_id": target.get('case_id', 'query_id_default'),
                "title": target.get('title', f"Query: {target_text[:30]}"),
                "year": target_year if target_year is not None else "N/A",
                "jurisdiction": target_jurisdiction
            },
            "retrieved_cases": retrieved_cases,
    ...
    def _compute_similarity(self, query: str, retrieved: list) -> list:
        """Return candidates with similarity scores from pre-retrieved (idx, score) pairs."""
        results = []
